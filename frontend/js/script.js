document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        if (clientNameEl) clientNameEl.textContent = `Olá, ${user.nome}!`
    ...
    function animateCount(el, start, end, duration) {
        ...
        function step(ts) {
            ...
            el.textContent = "R$ " + value.toLocaleString("pt-BR", { minimumFractionDigits: 2 })
    ...
    if (scoreCanvas && scorePointsLabel) {
        ...
        scorePointsLabel.textContent =
            `${score.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})} Pontos`
    }
    ...
    openDepositDialogBtn.addEventListener("click", () => depositDialog.showModal())
    cancelDepositBtn.addEventListener("click", () => depositDialog.close())
    ...
    confirmDepositBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            Swal.fire("Sucesso","Depósito realizado!","success").then(() => window.location.reload())
            depositDialog.close()
        } catch (err) {
    ...
    // 🔹 SAQUE
    const saqueDialog = document.getElementById("saqueDialog")
    const openSaqueDialogBtn = document.getElementById("openSaqueDialog")
    const cancelSaqueBtn = document.getElementById("cancelSaque")
    const confirmSaqueBtn = document.getElementById("confirmSaque")

    openSaqueDialogBtn.addEventListener("click", () => saqueDialog.showModal())
    cancelSaqueBtn.addEventListener("click", () => saqueDialog.close())

    confirmSaqueBtn.addEventListener("click", async () => {
        const value = Number(document.getElementById("saqueValue").value)
        if (!value || value <= 0) {
            Swal.fire("Erro","Informe um valor válido","error")
            saqueDialog.close()
            return
        ...
        if (value > saldo) {
            ...
            Swal.fire({ icon:"error", title:"Saldo insuficiente", text:`Seu saldo atual é R$ ${saldo.toLocaleString("pt-BR",{minimumFractionDigits:2})}`})
            saqueDialog.close()
            return
        }

        const originalText = confirmSaqueBtn.innerText
        confirmSaqueBtn.innerText = 'Processando...'
        confirmSaqueBtn.disabled = true

        try {
            const res = await fetch("http://127.0.0.1:8000/saques/", {
                method: "POST",
                headers: { "Content-Type":"application/json", "Authorization": `Bearer ${token}` },
                body: JSON.stringify({ valor: value })
            })
            const data = await res.json()
            if (!res.ok) {
                Swal.fire("Erro", data.detail || "Erro ao sacar","error")
                return
            }

            Swal.fire("Sucesso","Saque realizado!","success").then(() => window.location.reload())
            saqueDialog.close()
        } catch (err) {
            Swal.fire("Erro","Erro de conexão","error")
        } finally {
            confirmSaqueBtn.innerText = originalText
            confirmSaqueBtn.disabled = false
        }
    })
